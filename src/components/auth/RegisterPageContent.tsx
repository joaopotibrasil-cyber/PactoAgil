...
function RegisterForm() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      setPlanKey(searchParams.get("plan") || "");
    }
  }, []);
  ...
  // Lógica de Registro via API Astro
  const { state: registrationState, execute: runRegistration, isLoading } = useAsyncState(async (formData: any) => {
    ...
    if (!response.ok) throw new Error(result.error || 'Erro ao processar registro.');
    return result;
  });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (selectedCompany) {
      data.existingCompanyId = selectedCompany.id;
      data.companyName = selectedCompany.nome;
      data.cnpj = selectedCompany.cnpj;
    } else {
      data.companyName = manualCompanyName;
      data.cnpj = manualCnpj;
    }

    try {
      const result = await runRegistration(data);
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          >
            {isLoading ? (
              ...
              <>
                Confirmar Registro <ArrowRight className="w-5 h-5" />
              </>
            )}
          </button>
        </form>
...
export default function RegisterPageContent() {
  return (
    <main className="min-h-[100dvh] bg-background flex flex-col items-center justify-center relative overflow-x-hidden overflow-y-auto py-8 px-3 sm:px-4">
